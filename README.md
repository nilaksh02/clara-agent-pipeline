...
// US01 table — now built on the SHARED Table component (components/common/Table)
// per team convention, instead of raw <table> tags.
//
// Sort order is preserved: `customers` is already sorted upstream in Dashboard.js,
// and Table renders rows in the order given, so US01 AC2 still holds.
//
// Custom cells (avatar, priority badge, View button) are supplied via renderRow.
...
const COLUMNS = ["NAME", "PRIORITY", "DUE DATE", "CATEGORY", "REASON", "AMOUNT", "ACTION"];

// up-to-2-letter initials from a name, for the avatar
